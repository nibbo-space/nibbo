...
export function familyAchievementLabel(achievementId: string, language: AppLanguage) {
  const badges = I18N[messageLocale(language)].achievements.badges as Record<string, string>;
...
export function familyAchievementDescription(achievementId: string, language: AppLanguage) {
  const desc = (I18N[messageLocale(language)].achievements as { badgeDescriptions: Record<string, string> }).badgeDescriptions;
